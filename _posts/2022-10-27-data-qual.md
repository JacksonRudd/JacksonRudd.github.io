...
85% of the universe is constituted of dark matter -  that is, matter which does not interact with the electromagnetic spectrum and avoids detection. In much the same way, all datasets contain *dark issues*, data quality issues that cannot be directly observed. These issues are not just difficult to find, but actually undetectable.  

Indeed we will prove in this post that dark issues exist in all non-trivial datasets as a necessary result of an simple mathematical structure inherent to all data. 
