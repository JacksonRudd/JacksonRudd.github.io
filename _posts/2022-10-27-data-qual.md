...
For a problem with real-world assumptions $$g_1, g_2... g_M$$ we also consider the *assumption violation alert* (or $$cva$$) which fires when *any* assumption is violated. Formally we can define the *ava* as follows. 
...
This is because if the alert fires we have $$g(\hat{X}) = \text{False}$$. And the setup is plausible we have $$g(\hat{T}) = \text{True}$$.
