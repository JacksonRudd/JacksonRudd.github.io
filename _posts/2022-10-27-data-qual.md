...
> While data quality issues are detectable, data correctness is not. 

Physics provides a useful analogy for data quality: 85% of the universe consists of "dark matter" -  that is, matter which does not interact with the electromagnetic spectrum and avoids detection. 

In much the same way, all datasets contain *dark issues* - data quality issues that cannot be directly observed. These issues are not just difficult to find, but actually totally undetectable.  

Indeed we will prove in this post that dark issues exist in all non-trivial datasets as a result of a simple mathematical structure underlying data problems. 

Dark issues can be *partially* eliminated by making real-world assumptions on the data.  We will derive an equation that specifies the *exact* number of dark issues as a function of our real-world assumptions. 

The focal point of this analysis is the Maximal Alert Theorem, which says that the *only* way to determine quality issues is to look for violations of real-world assumptions. 

Probabilistic alerts are extremely useful for the discovery of quality issues, and will be covered in a future post building off of the epistemological theory developed here. 
...
## Data Quality Issues at the Ferry
...
The maximal alert is, by definition, the most comprehensive alert. No alert fires when the maximal alert is silent. It can be formally constructed by 'or'-ing all of the other alerts together. 
...
Every plausible state of the true values only has one row of observables with no data quality issues. Therefore there are only $$\lvert P \rvert$$ good data quality states. It follows that there exist  $$\lvert P \rvert  \lvert \hat{X} \rvert - \lvert P \rvert$$ bad data quality states. 
