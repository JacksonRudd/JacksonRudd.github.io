...
If you ask an analyst to find issues in a dataset, they will come back in a few hours with a long list. If you instead ask them to 'validate' a dataset it will take months... and issues will still be observed after validation. 
...
We can agree that if we have two deterministic alerts, $$a_1$$ and $$a_2$$, then we can define a new deterministic alert by checking if $$a_1$$ is violated *or* if $$a_2$$ is violated. Essentially we can 'or' two deterministic alerts together in a new deterministic alert. 
...
The quality issues that are not caught by the maximal alerts constitute the dark issues. How many issues are these. 
