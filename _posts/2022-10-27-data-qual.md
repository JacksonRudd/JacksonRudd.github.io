...
Why the long tail? I believe that much of the issue comes down to detectability of data quality issues. There are some accuracy issues that are totally undetectable. 
...
We will see that dark issues exist in all non-trivial datasets as a result of a simple mathematical structure underlying data problems. 

Dark issues can be *partially* eliminated by making real-world assumptions on the data.  We will develop an equation for enumerating  the number of dark issues as a function of our real-world assumptions. 
...
Probabilistic alerts are extremely useful for the discovery of accuracy issues and will be covered in a future post building off the epistemological theory developed here. 
