...
Physics provides a useful analogy for data quality: 85% of the universe consists of "dark matter" -  that is, matter which does not interact with the electromagnetic spectrum and avoids detection. In much the same way, all datasets contain *dark issues* - data quality issues that cannot be directly observed. These issues are not just difficult to find, but actually totally undetectable.  

Indeed we will prove in this post that dark issues exist in all non-trivial datasets as a necessary result of a simple mathematical structure inherent to all data. 

Dark issues can be partially eliminated by making real-world assumptions on the data. The more assumptions made, the less dark issues exist, but there will always be some undetectable issues. We will derive an equation that specifies the *exact* number of dark issues as a function of our real-world assumptions. 
