...
If you ask an analyst to find issues in a dataset, they will come back in a few hours with a long list. If you instead ask them to 'validate' a dataset it might take months, and issues will still be observed. 

While data issues are detectable, data correctness is not. 

85% of our universe is constituted of 'dark matter' -  that is, matter which does not interact with the electromagnetic spectrum and avoids detection. In much the same way, all datasets contain 'dark issues', data quality issues that cannot be directly observed. 

Indeed we will prove in this post that dark issues exist in all non trivial datasets as a necessary result of a mathematical structure inherent to all data. 

We can reduce the number of these issues by making real world assumptions on the data. The more assumptions we can make, the less dark-issues exist. 
We will derive an equation that specifies the *exact* number of issues as a function of our real world assumptions. 
...
We can agree that if we have two alerts, $$a_1$$ and $$a_2$$, then we can define a new alert by checking if $$a_1$$ is violated *or* if $$a_2$$ is violated. 
...
There will only ever be $$\lvert P \rvert$$ good data quality states. Therefore there is $$\lvert P \rvert  \lvert \hat{X} \rvert - \lvert P \rvert$$ bad data quality states. 
