...
### Definition Plausible Data Quality Issue 

If $$\hat{X}\neq \hat{T}$$ and $\hat{T}$ satisfies all constraints $$g_1, g_2.. g_M$$, we say this is a *plausible data quality issue*. 

We will only observe plausible issues, since non plausible issues violate our assumptions. 
...
When $$f(\hat{X}) $$ has value $$\text{True}$$ than there is a plausible data quality issue.
...
**Excercise:** prove $$a$$ is an alert (that $$a$$ only fires when there is a plausible data quality issue). 
