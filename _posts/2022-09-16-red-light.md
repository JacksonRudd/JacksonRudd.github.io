...
---
A lot of people would stop here. This is pretty intuitive, and the math works out very nicley. But we will venture further into the problem by relaxing our assumption that the probability is uniform. 

Intuitivley, we can maybe go faster by speeding up when we think the light is about to turn.This is only possible given non uniform probabilities. 
...
Given:
$$ \int_0^{T_{max}} v dt = D$$
This statement immediatly made me think of the Extremal Holder's inequality. 
